...
class CostaballenaProperty():
    ...
    def getProducts(self, imageDownload, records):
        ...
        if len(links) > 0:
            ...
            for link in links:
                ...
                try:
                    ...
                    data = self.getProductDetail(link, imageDownload)
                    if len(data) > 0:
    ...
    def getProductDetail(self, link, imageDownload):
        ...
        try: 
            ...
            p2number = product_soup.find("div", attrs={"class": "p2no"}).text
            fields['P2#'] = p2number

            # This is for getting product images info.
            # if imageDownload:
            #     self.getProductImages(product_soup, p2number)
        ...
        try:
            ...
            if imagesContainer:
                ...
                for image in images:
                    ...
                    self.writeImage(image_url, item_number)
                print('images saved successfully.')
        except:
            pass
    
    # method to write image in local system.
    def writeImage(self, img_url, item_number):
        try:
            ...
            if size: 
                ...
                while True:
                    ...
                    if p.image:
                        ...
                        filename_new = str(uuid.uuid4()) + str(file_extension)
                        image_path = './images/' + str(item_number)
                        if not os.path.exists(image_path):
                            os.makedirs(image_path)
                            
                        image_path = image_path + "/" + filename_new
    ...
    def start(self):
        imageDownload = True
        records = None
        # if '-nopics' in sys.argv[-1].encode('utf-8'):
        #     imageDownload = False
        products = self.getProducts(imageDownload, records)
        if len(products) > 0:
